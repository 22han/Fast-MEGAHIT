...
struct CountSketchInstance{
	...
	void storeValsInSketch(int hashbufidx) {//在每个线程下，用于计算k-mer哈希，确定级别

			ulong_t val; //存储从哈希数组中读取的值
			unsigned int W, col, pos, position;
			uint32 value;
			ulong_t  out[2]; //存储Murmur3哈希函数的输出

			const int incr = KmerHash::km_word_len;
			const int upper = KmerHash::km_hash_idx_arr[hashbufidx];//指向哈希缓冲区特定的指针
			const int murmsize = KmerHash::km_word_len << 3;//k-mer位数
			const unsigned int seed = rand.murmseed;
				for (int i=0; i<= upper; i+=incr) {

					MurmurHash3_x64_128 ( &(KmerHash::km_hash_vals_arr[hashbufidx][i]), murmsize, seed,  &out );//hash之后确定级别
					val = out[0];

					W = index64[((val & -val) * debruijn64) >> 58];//分级别
					...
					if(sketch[W] != NULL) {
						...
						sketch[W][col] = (pos << 16) | 1; //pos是为了进一步区分不同k-mer，value代表个数
					}
				}
			}
	...
	void analyzeSketch(){ 
		...
		for (int i=0; i<=buckets; i=i+1) {//假设桶的个数代表M个级别,选一个最好的级别来估计呀
			...
			for (int b=0; b<=nf; b=b+1) {//更新这个级别的数据，和最好的比较
				if(b == 0){ 
	...
	ulong_t computeF0(){//计算是对的

		if(finfo_arr[0].t_opt == 0) {//相当于一点都没有
			finfo_arr[0].F = 0;
	...
	ulong_t computeFJ(int j, long F0){ //
		...
		double x = (double) R;
		x = 1 - (1/x);
		double exp = (double) ((double)F0/ (double)(1L << w_opt));
		double p0 = pow(x, exp); 
		//double pj = (double) ((double) t_opt/ (double) R);
		double val = (double)(1L<< w_opt);
		val=val*t_opt*x/p0;//val = val * ((double) (R -1)) * (pj / p0);
