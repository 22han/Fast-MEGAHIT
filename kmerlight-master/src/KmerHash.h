...
struct KmerHash{
		...
		static void init(unsigned int k){ 
			km_len = k;
			km_word_len = (km_len >> 5) + ((km_len & 31) > 0 ? 1 : 0); //31=11111 取km_len最后5位，表示处理一个缓冲区需要的单词数

			last_segment_status = ON_COMMENT_LINE;//跟踪最后一个数据段的状态


			km_hash_vals_arr[0] = new ulong_t[Constants::seq_buf_size * km_word_len];
			km_hash_vals_arr[1] = new ulong_t[Constants::seq_buf_size * km_word_len]; //  两个哈希缓冲区在分配内存，用于存储序列中每个k-mer的哈希值


			km_buf_arr = new ulong_t[2*km_word_len](); //存储当前处理的k-mer
			km_rev_buf_arr = km_buf_arr + km_word_len; //存储反向k-mer
		...
		static void computeLmask(){
			...
			rev_shift = (km_len<<1) -2;//考虑反向互补序列*2-2
			lmask = (k1 == 0)? (0xffffffffffffffffUL) : (ulong_t) ( (((ulong_t)1) << (2*k1)) -1 );//lmask=2*k1位的掩码，全是1，一个k-mer占两位的
		}




		INLINE static int lShiftSeqHashPartSingle(byte_t*  seqbuf, int & start, int & end, int & idx, int hashbufidx){//只是将k-mer存起来了
			register ulong_t c=0;
			ulong_t * km_hash_vals = km_hash_vals_arr[hashbufidx];//只有0，1
			...
			while((idx <= end)  && ((c = Kmer::bpval(seqbuf[idx])) <= 3)){//是ATCG

				km_buf = ((km_buf << 2) & lmask) | c;//每个碱基用两位表示，把k-mer存起来
			...
			return ((idx > end)? _valEOFBUF: c);//返回值是c=\n
		}
...
//####################  TWO  #####################################
		...
		INLINE static void resetKMBuf(int hashbufidx){ //重置哈希
