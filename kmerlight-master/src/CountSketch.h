...
struct CountSketch{
	...
	void storeValsInSketchMT(int hashbufidx){//第一个线程调用的函数

		if(thrd_data_arr == NULL){//这里建立多个线程
			thrd_data_arr = new CSthrdData[Constants::nthreads];//每个线程代表实例
	...
	void storeValsInSketch(int hashbufidx){
		for(int i=0; i< Constants::copies; ++i){
			sketch_arr[i].storeValsInSketch(hashbufidx);//就是确定每个k-mer的w和位置
		}
	}
	...
	static ulong_t * computeAllF(int num_freq){//开始计算过程
